...
const TT_BORDER_COLORS=['#FFFFFF','#D1D5DB','#6B7280','#26262C','#60A5FA','#93C5FD'];
function defaultTimetable(){
const E=(day,start,end,course,room,color)=>({id:uid(),day,start,end,course,room,color,text:null});
return{section:'B',break:'11:10–11:25',lunch:'1:05–1:45',appearance:{text:'#EDEBE8',background:'#EAF4FF',border:'#FFFFFF',showBorder:false},entries:[
...
if(!ap.text)ap.text=def.text;if(!ap.background)ap.background=def.background;if(!ap.border)ap.border=def.border;if(typeof ap.showBorder!=='boolean')ap.showBorder=def.showBorder;
return false;}
...
const mobileRows=TT_SLOTS.map(s=>{const e=ttAt(mobileDay,s.id);const start=e&&e.start===s.id;return '<div class="tt-mobile-slot"><div class="tt-mobile-time"><b>'+s.label+'</b><small>'+s.time+'</small></div><div class="tt-mobile-class">'+(start?'<button data-a="tt-edit" data-id="'+e.id+'" style="--ttc:'+esc(e.color||TT_COLORS[0])+';--ttx:'+esc(ttTextFor(e))+'"><b>'+esc(e.course)+'</b>'+(e.room?'<small>Room '+esc(e.room)+'</small>':'')+'</button>':(e?'':'<button data-a="tt-new-slot" data-day="'+mobileDay+'" data-start="'+s.id+'" class="tt-mobile-empty">Free</button>'))+'</div></div>';}).join('');
const mobile='<div class="tt-mobile-view">'+mobileNav+'<div class="tt-mobile-list">'+mobileRows+'</div></div>';
...
openModal('<div class="m-head"><h2>'+(old?'Edit class':'Add class')+'</h2><button class="icon-btn" data-a="close-modal" aria-label="Close">'+ico('x')+'</button></div><div class="m-body"><label class="fld"><span>Course / Class</span><input class="input" id="tt-course" value="'+esc(f.course)+'" placeholder="e.g. EAI"></label><div class="frow"><label class="fld"><span>Day</span><select class="select" id="tt-day">'+days+'</select></label><label class="fld"><span>Room</span><input class="input" id="tt-room" value="'+esc(f.room||'')+'" placeholder="e.g. 107"></label></div><div class="frow"><label class="fld"><span>Start</span><select class="select" id="tt-start">'+opts(f.start)+'</select></label><label class="fld"><span>End</span><select class="select" id="tt-end">'+opts(f.end)+'</select></label></div><label class="fld"><span>Block color</span><select class="select" id="tt-color">'+colors+'</select></label><div class="tt-class-color"><span>Text colour</span><div class="tt-swatches" id="tt-text-sw">'+textColors+'</div><small class="hint">Leave unselected to use automatic contrast.</small></div></div><div class="m-foot">'+(old?'<button class="btn btn-danger" data-a="tt-del">Delete</button>':'')+'<span class="grow"></span><button class="btn" data-a="close-modal">Cancel</button><button class="btn btn-primary" data-a="tt-save">Save class</button></div>',{label:'Timetable class'});}

function openTimetableTheme(){
